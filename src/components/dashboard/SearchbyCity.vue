<template>
  <div class="flex w-full max-w-[700px] mx-auto items-center justify-center">
    <div
      ...
      class="fixed top-0 left-0 bottom-0 right-0 z-[4]"
    ></div>
    <div class="relative w-full max-w-[700px] z-[20]">
      <div
        class="px-4 w-full py-2 max-w-[700px] flex items-center rounded-md bg-mainblue/10"
      >
        <input
          @focus="() => (state.hide = false)"
          v-model="state.input"
          class="w-full cityinput outline-none pr-2"
          type="text"
          placeholder="Search Places By City"
        />
        <v-icon
          class="text-mainblue text-base -translate-y-1"
          icon="fas fa-search"
        />
      </div>
      <div
      ...
      >
        ...
        <p
          v-if="state.input.trim() === '' || state.loaded.length === 0"
          class="text-sm text-center"
        >
          Type a city to search places in
        </p>
        <button
          @click="showMore"
          v-if="!(state.shown.length === state.loaded.length)"
          class="text-sm text-mainblue w-fit ml-3"
        >
          Show more
        </button>
      </div>
...
watch(
  ...
  (val) => {
    ...
    const loaded = state.cities.filter((city) =>
...
onMounted(() => {
  state.cities = City.getAllCities().filter(
