      ...
      <div class="flex flex-col w-full gap-y-4">
        ...
        <div class="flex flex-col w-full max-w-[500px]">
          ...
          <input
            ...
            class="w-full rounded-lg outline-none focus:border-mainblue duration-300 px-2 py-2 border-2 border-mainblue/30 border-solid mt-2"
            autocomplete="false"
            v-model="newInfo.confirmPassword"
            placeholder="New Passwprd"
          />
        </div>
        ...
        <button
          ...
          :class="`${
            status.loading ? 'bg-mainblue/50' : 'bg-mainblue/90'
          } rounded-lg duration-300 hover:bg-mainblue w-fit px-5 py-2 hover:shadow-md shadow-mainblue text-white`"
        >
          Save Changes
        </button>
      </div>
...
const changePassword = async () => {
  ...
  status.loading = true;
  const data = await userStore.updatePasword(
    newInfo.oldPassword,
    newInfo.newPassword
  );
  if (data.error) {
